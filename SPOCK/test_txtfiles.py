...
def startup(t_now,name,sun_set,date_start,Path,telescope):
    ...
    if telescope == 'Saint-Ex':
        with open(os.path.join(Path,str(t_now),'startup_' + datetime.strptime(t_now,'%Y-%m-%d').strftime('%m%d%Y') + '.txt'),'w') as out:
            ...
            start_time_saint_ex = Time((observatory.twilight_evening_nautical(Time(t_now)+1,which='nearest').jd + Time(sun_set).jd)/2,format='jd').iso
            out.write(';' + str0 + '\n')
            for i in range(1,3):
                out.write(str00)
            ...
            for i in range(1,2):
                out.write(str00)
            out.write(str1 + start_time_saint_ex + '\n')#str(hour1) + ':' + str(minute1) + '\n')
            out.write(str4)
            for i in range(1,2):
                out.write(str00)
    ...
    if telescope == 'Saint-Ex':
        with open(os.path.join(Path, str(t_now), 'Cal_flatdawn' + '_' + datetime.strptime(t_now, '%Y-%m-%d').strftime('%m%d%Y') + '.txt'), 'w') as out:
            ...
            end_time_saint_ex = Time((observatory.twilight_morning_nautical(Time(t_now)+1,which='next').jd +
                                      observatory.twilight_morning_civil(Time(t_now)+1,which='next').jd)/2,format='jd').iso
            out.write(str1 + str(hour0) + ':' + str(minute0) + '\n')
            out.write(str3 + str4)
            out.write(str44 + end_time_saint_ex + '\n')
            out.write(str5 + str6)
    else:
...
def biasdark(t_now,Path,telescope):
    if telescope == 'Saint-Ex':
